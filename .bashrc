...
git_is_worktree() {
    ...
    local common=$(git rev-parse --git-common-dir 2>/dev/null)
    [[ "$dir" == "$common" ]] && return 1
    return 0
}
git_branch() {
...
get_dir() {
    if git_is_worktree; then
        local branch_path=$(git rev-parse --show-toplevel 2>/dev/null)
        basename ${PWD/$branch_path/${branch_path%/*}}
    else
        basename $(dirs)
    fi
}
...
if [[ $TMUX ]]; then
    PS1="\[\e]0;\u@\h: \w\a\]\${debian_chroot:+(\$debian_chroot)}\[\033[01;34m\]\$(get_dir)\[\033[00m\]\$(__git_ps1)\$ "
else
    PS1="\[\e]0;\u@\h: \w\a\]\${debian_chroot:+(\$debian_chroot)}\[\033[01;32m\]\u@\h\[\033[00m\]:\[\033[01;34m\]\w\[\033[00m\]\$(__git_ps1)\$ "
fi

export PATH="$PATH:/opt/nvim-linux64/bin:$HOME/go/bin:/usr/local/go/bin"
export EDITOR="nvim"

function load_completions() {
    [ -d $HOME/.bash_completion.d ] && for file in "$HOME/.bash_completion.d/*"; do
        if [ -r $file ]; then
            . $file
        fi
    done
